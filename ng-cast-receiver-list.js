	angular.module('crl', [])
		.factory('crl.receiverService', [ 'castApi', 'CAST_APP_ID', function (castApi, appId) {
			return {
				addReceiverListener: function (callback) {
					castApi.ready(function (api) {
						api.addReceiverListener(appId, callback);
					});
				}
			};
		}])
		.controller('crl.ReceiverCtrl', [ '$scope', 'crl.receiverService', function(scope, receiverService) {
			receiverService.addReceiverListener(function (receivers) {
				scope.$apply(function () { scope.receivers = receivers; });
			});
		}])
